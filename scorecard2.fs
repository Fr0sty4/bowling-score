...
let evaluateBowl pins game =
    ...
    | FrameR Unplayed::rest -> if pins=10 then FrameR Strike::rest else FrameR (InProgress pins)::rest
    | FrameR (InProgress p1)::rest -> if pins+p1=10 then FrameR (Spare (p1,pins))::rest else FrameR (Complete (p1,pins))::rest
    // Cases for the final frame
    | FrameF UnplayedF::rest -> if pins=10 then FrameF (InProgressBonusF1 pins)::rest else FrameF (InProgressF pins)::rest
    | FrameF (InProgressF p1)::rest -> if pins+p1=10 then FrameF (InProgressBonusF2 (p1,pins))::rest else FrameF (CompleteF (p1,pins))::rest
